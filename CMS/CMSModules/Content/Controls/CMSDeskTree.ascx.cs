            ...
            case "sortdatedesc":
                ...
                try
                {
                        ...
                        && (currentUser.IsAuthorizedPerDocument(node, NodePermissionsEnum.ExploreTree) == AuthorizationResultEnum.Allowed))
                    {
                        ...
                        {
                            case "sortalphaasc":
                                tree.SortNodesAlphabetically(nodeId, true);
                                break;

                            case "sortalphadesc":
                                tree.SortNodesAlphabetically(nodeId, false);
                                break;

                            case "sortdateasc":
                                tree.SortNodesByDate(nodeId, true);
                                break;

                            case "sortdatedesc":
                                tree.SortNodesByDate(nodeId, false);
                                break;
                        }
                ...
                {
                    EventLogProvider.LogEvent(EventType.ERROR, "Content", "SORT", EventLogProvider.GetExceptionLogMessage(ex), RequestContext.RawURL, currentUser.UserID, currentUser.UserName, nodeId, documentName, RequestContext.UserHostAddress, SiteContext.CurrentSite.SiteID);
