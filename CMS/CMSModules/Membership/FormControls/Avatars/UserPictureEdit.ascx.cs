...
public partial class CMSModules_Membership_FormControls_Avatars_UserPictureEdit : FormEngineUserControl
{
    ...
    private UserInfo FormUserInfo
    {
        get
        {
            if ((mFormUserInfo == null) && (Data != null))
            {
                ...
                int userID = ValidationHelper.GetInteger(Data["UserID"], 0);
                if (userID > 0)
                {
                    mFormUserInfo = UserInfoProvider.GetUserInfo(userID);
                }
            }
    ...
    private string GetAvatarType()
    {
        ...
        string avatarType = DataHelper.GetNotEmpty(SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".CMSAvatarType"), AvatarInfoProvider.AVATAR);
    ...
    private void UpdateAvatar()
    {
        ...
        if (IsNewPictureUploaded())
        {
            avatar = StoreUploadedPicture(avatarID);
        }
        ...
        if (FormUserInfo != null)
        {
            ...
            FormUserInfo.UserSettings.UserAvatarType = rdbMode.SelectedValue;
            FormUserInfo.UserAvatarID = avatarID;
            UserInfoProvider.SetUserInfo(FormUserInfo);
        }
    }
    ...
    /// <returns>Stored avatar info.</returns>
    private AvatarInfo StoreUploadedPicture(int avatarId = 0, UserInfo user = null)
    {
    ...
    private void ShowUserAvatar(UserInfo user)
    {
        ...
        if (user.UserAvatarID > 0)
        {
            ...
            btnDeleteImage.Visible = (user.UserSettings.UserAvatarType == AvatarInfoProvider.AVATAR);
        }
    }
