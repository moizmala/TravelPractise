    ...
    private void LoadObjects()
    {
        if (Object != null)
        {
            ...
            if (DisplayNewButton)
            {
                ...
                {
                    mJavaScript.Append("function US_NewItem_", UniSelectorClientID, "(selectedItem) {{ var url = '", ScriptHelper.ResolveUrl(newUrl), "';selectedItem = US_TrimSeparators(selectedItem, '", ValuesSeparator, @"');modalDialog(url.replace(/##ITEMID##/i, selectedItem),'NewItem', ", EditDialogWindowWidth, ", ", EditDialogWindowHeight, "); }} \n");
                }
            }
...
");
    ...
    {
        return (Object != null) ? new WhereCondition().WhereContains(Object.DisplayNameColumn, cond).ToString(true) : null;
    }
    ...
    /// </summary>
    private ListItem NewListItem(string text, string value)
    {
...
null), Page.ClientScript.GetPostBackEventReference(this, "selectionchanged"), AllowEmpty.ToString().ToLowerCSafe(), String.IsNullOrEmpty(mInitSet) ? "''" : mInitSet, useCallback.ToString().ToLowerCSafe(), events,
    ...
    private string GetItemName(DataRow dr)
    {
        ...
        {
            itemName = ResHelper.LocalizeString(ValidationHelper.GetString(DataHelper.GetDataRowValue(dr, obj.DisplayNameColumn), String.Empty));
        }
        else
        {
        ...
        if (AddGlobalObjectSuffix)
        {
            ...
            if ((obj != null) && (ti.SiteIDColumn != ObjectTypeInfo.COLUMN_NAME_UNKNOWN))
            {
                itemName += (ValidationHelper.GetInteger(DataHelper.GetDataRowValue(dr, ti.SiteIDColumn), 0) > 0 ? String.Empty : " " + GlobalObjectSuffix);
            }
        }
