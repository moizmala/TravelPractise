...
function AutoCompleteExtender(codeElem, hintElem, contextElem, quickContextElem, mixedMode, editorTopOffset, editorLeftOffset, ascxMode) {
    ...
    */
    ...
    this.positionDivs = function () {
        ...
        // Set position
        var x = Math.min(caretPos.x - offset, xMax);
        var y = caretPos.y;
        ...
        if (hintDown && ($cmsj(document).height() < y + 25 + autocompleteHeight) && (0 <= y - offset - autocompleteHeight)) {
            hintDown = false;
        }

        if (!hintDown && (0 > y - offset - autocompleteHeight) && ($cmsj(document).height() > y + 25 + autocompleteHeight)) {
            hintDown = true;
        }

        // Subtracts the window scrollbar and converts to viewport coords.
        y -= $cmsj(window).scrollTop();
        ...
        if (hintDown) {
            ...
            this.contextDiv.style.left = contextX + unit;
            this.contextDiv.style.top = y + unit;
        ...
        this.hintsDiv.style.left = x + unit;
        this.hintsDiv.style.top = y + unit;
    ...
    this.positionQuickContextDiv = function () {
        ...
        if (this.hasVerticalScrollbar()) {
            var z = (window.innerWidth - (pos.right + offset)) - 36;
        }
        else {
            var z = (window.innerWidth - (pos.right + offset)) - 20;
        }
    ...
    this.hasVerticalScrollbar = function () {
        if ($cmsj('.PageBody')[0] != null) {
            var scrollHeight = $cmsj('.PageBody')[0].scrollHeight;
        }
            // E.g. field macro edit window
        else {
            var scrollHeight = $cmsj('.DialogPageBody')[0].scrollHeight;
        }
